...
tr0 <- read_csv(file.path(here::here(), 'data/train_v2.csv'), col_types = ctypes)
te0 <- read_csv(file.path(here::here(), 'data/test_v2.csv'), col_types = ctypes)
```
...
```{r}
saveRDS(train_prc, file.path(here::here(), 'data/train_prc.rds'))
```
...
```{r}
saveRDS(test_prc, file.path(here::here(), 'data/test_prc.rds'))
```

```{r}
###Defining useful columns
good_cols = c("channelGrouping",
              "date","fullVisitorId","visitId","visitNumber","visitStartTime","browser",
              "deviceCategory",
              "isMobile","operatingSystem","city","continent","country","metro",
              "networkDomain","region","subContinent","bounces",                              
              "hits","newVisits","pageviews","sessionQualityDim","timeOnSite",
              "totalTransactionRevenue","transactionRevenue",
              "transactions","adContent",
              "adwordsClickInfo.adNetworkType", "adwordsClickInfo.gclId","adwordsClickInfo.isVideoAd",
              "adwordsClickInfo.page",
              "adwordsClickInfo.slot","campaign","isTrueDirect","keyword","medium","referralPath","source"
              )
```

```{r}
full = rbind(train_prc[c(good_cols)], test_prc[c(good_cols)])
full = full %>% as_tibble()
```
...
```{r}
full_df$transactionRevenue = ifelse(is.na(full_df$transactionRevenue) == TRUE, 0, full_df$transactionRevenue)
```
...
getTimeFramewithFeatures <- function(data, k)
{
  ...
  tf = plyr::join(tf, tf_target, by = "fullVisitorId")
  return(tf)
}
```
...
tr1 = getTimeFramewithFeatures(full_df, 1)
message("Get 2nd train part")
tr2 = getTimeFramewithFeatures(full_df, 2)
message("Get 3rd train part")
tr3 = getTimeFramewithFeatures(full_df, 3)
message("Get 4th train part")
tr4 = getTimeFramewithFeatures(full_df, 4)
```

```{r}
###Costruction of the test-set (by analogy as train-set) 
message("Get test")
tr5 = full_df[full_df$date >=  '2018-05-01',]
...
write.csv(newsub4, file.path(here::here(), "data/tst4.csv"), row.names = FALSE, quote = FALSE)
```
